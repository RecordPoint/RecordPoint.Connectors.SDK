...
namespace RecordPoint.Connectors.Client
{
    public class ApiClientFactory : IApiClientFactory
    {
        ...
        public async Task<AuthenticationResult> AcquireTokenAsync(ApiClientFactorySettings settings, bool useTokenCache = true)
        {
            ...
            ValidationHelper.ArgumentNotNullOrWhiteSpace(settings.ClientSecret, nameof(settings.ClientSecret));
        ...
        private async Task<IApiClient> CreateApiClientAsync(ApiClientFactorySettings settings, bool useTokenCache = true)
        {
            ...
            var submissionUrl = new Uri(settings.ConnectorApiUrl);
            if(!settings.ConnectorApiUrl.Contains(ConnectorApiPrefix))
            {
                submissionUrl = new Uri(submissionUrl, ConnectorApiPrefix);
            }

            // get the api client generated by AutoRest from the swagger...
            var apiClient = new ApiClient(submissionUrl, new TokenCredentials(authResult.AccessToken, authResult.AccessTokenType));
